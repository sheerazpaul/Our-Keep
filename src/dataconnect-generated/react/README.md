...
## CreateNewNote
You can execute the `CreateNewNote` Mutation using the `UseMutationResult` object returned by the following Mutation hook function (which is defined in [dataconnect-generated/react/index.d.ts](./index.d.ts)):
```javascript
useCreateNewNote(options?: useDataConnectMutationOptions<CreateNewNoteData, FirebaseError, CreateNewNoteVariables>): UseDataConnectMutationResult<CreateNewNoteData, CreateNewNoteVariables>;
```
You can also pass in a `DataConnect` instance to the Mutation hook function.
```javascript
useCreateNewNote(dc: DataConnect, options?: useDataConnectMutationOptions<CreateNewNoteData, FirebaseError, CreateNewNoteVariables>): UseDataConnectMutationResult<CreateNewNoteData, CreateNewNoteVariables>;
```
...
```
### Return Type
Recall that calling the `CreateNewNote` Mutation hook function returns a `UseMutationResult` object. This object holds the state of your Mutation, including whether the Mutation is loading, has completed, or has succeeded/failed, among other things.

To check the status of a Mutation, use the `UseMutationResult.status` field. You can also check for pending / success / error status using the `UseMutationResult.isPending`, `UseMutationResult.isSuccess`, and `UseMutationResult.isError` fields.

To execute the Mutation, call `UseMutationResult.mutate()`. This function executes the Mutation, but does not return the data from the Mutation.

To access the data returned by a Mutation, use the `UseMutationResult.data` field. The data for the `CreateNewNote` Mutation is of type `CreateNewNoteData`, which is defined in [dataconnect-generated/index.d.ts](../index.d.ts). It has the following fields:
```javascript
...
### Using `CreateNewNote`'s Mutation hook function

```javascript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, CreateNewNoteVariables } from '@dataconnect/generated';
import { useCreateNewNote } from '@dataconnect/generated/react'

export default function CreateNewNoteComponent() {
  // Call the Mutation hook function to get a `UseMutationResult` object which holds the state of your Mutation.
  const mutation = useCreateNewNote();

  // You can also pass in a `DataConnect` instance to the Mutation hook function.
  const dataConnect = getDataConnect(connectorConfig);
  const mutation = useCreateNewNote(dataConnect);

  // You can also pass in a `useDataConnectMutationOptions` object to the Mutation hook function.
  const options = {
    onSuccess: () => { console.log('Mutation succeeded!'); }
  };
  const mutation = useCreateNewNote(options);
  ...
  const mutation = useCreateNewNote(dataConnect, options);

  // After calling the Mutation hook function, you must call `UseMutationResult.mutate()` to execute the Mutation.
  // The `useCreateNewNote` Mutation requires an argument of type `CreateNewNoteVariables`:
  const createNewNoteVars: CreateNewNoteVariables = {
    title: ..., 
    content: ..., 
    isFavorite: ..., // optional
  };
  mutation.mutate(createNewNoteVars);
  ...
  mutation.mutate(createNewNoteVars, options);
...
export default function UpdateNoteComponent() {
  ...
  // Variables can be defined inline as well.
  mutation.mutate({ id: ..., title: ..., content: ..., isFavorite: ..., });
